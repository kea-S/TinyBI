...
ssh "$REMOTE" \
  "nohup srun --gpus=2 --constraint=xgpf --time=02:00:00 \
   bash -c 'source ~/tesla_vllm_env/bin/activate && cd ~/tesla_vllm && VLLM_USE_FLASHINFER_SAMPLER=0 VLLM_ATTENTION_BACKEND=XFORMERS vllm serve ibm-granite/granite-4.1-3b --port 8001 --enable-auto-tool-choice --tool-call-parser granite4 --gpu-memory-utilization 0.85 --max-model-len 16384' \
   > /tmp/vllm.log 2>&1 &"
